...
## calculate beta coefficients for each subject and each ROI
...
betas_df$diff = betas_df$beta_face - betas_df$beta_object
betas_df$sum = betas_df$beta_face + betas_df$beta_object
...
for (roi_itr in seq_along(ROI_list)) {
  ...
  print(t.test (curr_betas$beta_object, mu=0))
  print(t.test (curr_betas$beta_face, mu=1))
  print(t.test (curr_betas$beta_object, mu=1))
...
### plot Fig. 5b
# function - plot group mean and s.e.m.
plot_means = function(curr_betas_mean,  curr_betas_sem, color_vec_all, fig_title){
  ...
  gg = gg + geom_point(data = curr_betas_mean,
                       aes(x = beta_face, y = beta_object, color = ROI),
                       size = 8,
                       shape = 18) +
    ...
    geom_errorbar(data = curr_betas_mean,
                  aes(x=curr_betas_mean$beta_face, 
                      min=beta_object - curr_betas_sem$beta_object,
                      ymax=beta_object + curr_betas_sem$beta_object),
                  width=.05) +
    
    geom_errorbarh(data = curr_betas_mean,
                   aes(y= curr_betas_mean$beta_object, 
...
# function - add single subject data to the mean figure
...
color_vec_all = c("medial_obj" = "limegreen",
                  "FFA" = "#2980b9")


curr_betas_mean = filter(betas_mean, ((ROI=="FFA" | ROI=="medial_obj" ) & hemi=="Right"))
curr_betas_sem = filter(betas_sem, ((ROI=="FFA" | ROI=="medial_obj" )& hemi=="Right"))
curr_betas_df = filter(betas_df, ((ROI=="FFA" | ROI=="medial_obj" )& hemi=="Right"))
...
rm(gg)
gg = plot_means(curr_betas_mean, curr_betas_sem, color_vec_all, "FO_Right Ventral FFA medial_obj lm")
add_subjects(curr_betas_df, gg, "FO_Right Ventral FFA medial_obj lm")

write.csv(curr_betas_df, "Fig5_data_2_FO.csv")
