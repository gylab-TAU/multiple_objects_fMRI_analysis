...
parameters {
  ...
  real beta_1_raw[L];
  real beta_2_raw[L];
  real beta_intercept_raw[L];
  real<lower=0> sigma_subj;
}
transformed parameters {
  real beta_1[L];
  real beta_2[L];
  real beta_intercept[L];
  real mu_sum;
  real mu_diff;
  for (l in 1:L){
    ...
    beta_intercept[l] = mu_intercept + sigma_intercept * beta_intercept_raw[l];
  }  
...
model {
  ...
  for (l in 1:L) {
    ...
    target += normal_lpdf (beta_intercept_raw[l] | 0, 1);
//    target += cauchy_lpdf(sigma_subj[l] | 0, 20) - cauchy_lccdf(0 | 0, 20);
  }
  for (n in 1:N)
    target += normal_lpdf (y[n] | (beta_intercept_raw[ll[n]] + beta_1[ll[n]] * x1[n] + beta_2[ll[n]] * x2[n]), sigma_subj);
}
